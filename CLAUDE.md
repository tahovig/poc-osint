...
- Working directory is now `~/dev-projects/tah-osint-poc` (WSL native filesystem) — this is the real, permanent working folder. The earlier Windows scaffold at `D:\tah-projects\poc-osint` is abandoned; nothing from it has been carried over.
- Directory currently contains only this `CLAUDE.md`. No git history, no README, no `.gitignore`, no `code/`/`resources/` subfolders yet — those need to be (re)created here.
...
1. Set up `main` \+ `develop` branches on GitHub and push initial scaffold.
2. Scaffold repo: `README.md`, `.gitignore` (Python-specific), `code/`, `resources/`, `git init`.
3. Build local Docker/HTTP-server test fixtures (needed for CI since no live target is set up yet).
