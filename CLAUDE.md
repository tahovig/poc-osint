...
- `src/poc_osint/crtsh.py` — crt.sh client, two independent fetch paths sharing one parser. `fetch_crtsh_json()` (network layer: GET with timeout + exponential-backoff retry, `CrtShError` after 3 failed attempts) and `fetch_crtsh_postgres()` (direct Postgres query against crt.sh's public `certwatch` DB — query is crt.sh's own, extracted via their `showSQL=Y` debug option, trimmed to just `NAME_VALUE`) both return the same `{"name_value": ...}` shape, so `extract_subdomains()` (pure parsing: dedupes, lowercases, strips `*.` wildcard prefixes, filters to the queried domain + its subdomains) handles either unchanged — including filtering out the CA-name/email-SAN noise the looser Postgres full-text query surfaces. `get_subdomains()` tries HTTP first, falls back to Postgres on `CrtShError`, raises (mentioning both) only if both fail.
...
- `src/poc_osint/recon.py` — orchestration layer. `run_recon(domain, *, max_concurrency, delay)`: crt.sh → liveness → header analysis, returns `list[SubdomainReport]`. Skips liveness entirely if crt.sh finds zero subdomains (returns `[]`, not an error) rather than inventing a fallback target.
...
User asked to keep visualization in mind while building CI; this is that feature, built afterward. Two parts, both terminal/ASCII-based (no charting dependency, consistent with the project's dependency-light style):
