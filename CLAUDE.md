...
- `src/poc_osint/cli.py` — argparse-based CLI, subcommand-structured (`poc-osint lookup <target>`). Only real functionality so far: domain-format validation; recon logic itself is not yet wired in (prints a placeholder message) — crt.sh/liveness/headers modules all exist now but aren't called from `cli.py` yet.
...
- `src/poc_osint/headers.py` — header parser, implemented. Pure/synchronous, no network — takes the `dict[str, str]` from `HostResult.headers`. `find_missing_security_headers()` checks the fixed 5-header checklist (CSP, HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy) case-insensitively; `fingerprint_server()` string-matches `Server`/`X-Powered-By` values against a short fixed signature list (Apache, nginx, IIS, PHP, ASP.NET, Express, Cloudflare) — deliberately not open-ended. `analyze_headers()` wires both into a `HeaderFindings` dataclass.
- `tests/unit/` — `test_cli.py`, `test_crtsh.py`, `test_liveness.py`, `test_headers.py` — 23/23 passing, `respx`-mocked where HTTP is involved, no live network. `test_headers.py` needs no mocking at all (pure functions, plain dict fixtures).
- `tests/integration/` — real Docker-fixture tests, implemented (`conftest.py` session fixture runs `docker-compose up`/`down` around the tests, polling until hosts respond). `test_liveness_fixtures.py` + `test_headers_fixtures.py` — 6/6 passing against real containers: healthy/vulnerable header profiles (liveness + header-parser both agree), dead-port detection, concurrent multi-port checks. Excluded from the default `pytest` run (`addopts = "-m 'not integration'"` in `pyproject.toml`, marker registered) — run explicitly via `pytest -m integration`, requires `dockerd` running. (crt.sh has no integration-test equivalent — it's an external CT log service, not something meaningfully containerizable locally; its "real" verification is the one-time manual live check below, not a fixture.)
...
- Verified end-to-end: `poc-osint-venv/bin/poc-osint lookup example.com` (exit 0), invalid domain (exit 1, stderr message), `pytest` — 23/23 unit passing, 6/6 integration passing. One-time manual live check of crt.sh's `get_subdomains()` against real crt.sh hit a genuine crt.sh outage (502, confirmed independently via `curl` — not a client bug); retry once crt.sh is back up to confirm real-world parsing.

## Open decisions / immediate next steps

1. Wire crt.sh + liveness + header parser into `lookup`, add structured JSON + table output — the last integration step before this is a working end-to-end tool.
