...
- **Verified fully end-to-end against live data**: `poc-osint-venv/bin/poc-osint lookup example.com` — real crt.sh query found 6 subdomains, real liveness checks correctly separated live (`example.com`, `www.example.com`, both ports) from dead (`dev.`, `m.`, `products.`, `support.` — DNS resolution failures), real header analysis correctly flagged Cloudflare fingerprint + 5 missing security headers on the live hosts (checklist grid renders as all-`✗` for those two, all-`-` for the dead ones). Both table and `--json` output modes confirmed. `pytest` — 59/59 unit passing, 6/6 integration passing.
- **Spinner verified live under a real pseudo-tty** (`script -qec ... /tmp/log`, since a piped/captured shell isn't a tty and would silently skip it): frames genuinely cycle with `\r` overwrites, stage text updates from "Querying crt.sh..." to "Checking liveness of 6 subdomain(s)...", final table renders with no leftover spinner residue. Note: the "Analyzing headers..." stage message never visibly renders in practice — that phase is synchronous over already-fetched data and finishes faster than the spinner's 80ms tick, so a frame is set but the task never gets scheduled again before `stop()` cancels it. Benign (the network-bound stages are what actually needed visible progress) but worth knowing if this surprises anyone reading the code. Also verified piped output (`--json > file`) produces zero stderr bytes — the `isatty()` gate works.
