...
The core tool is functionally complete end-to-end (crt.sh → liveness → headers → output), with CI green on GitHub and a working crt.sh fallback path. `main` and `develop` are in sync (fast-forward merged). Remaining items:
1. Results visualization/comparison across runs (see Future consideration above) — not scoped/prioritized yet.
2. README doesn't yet mention the Postgres fallback exists — minor doc gap, not urgent since it's fully transparent to the CLI user.
